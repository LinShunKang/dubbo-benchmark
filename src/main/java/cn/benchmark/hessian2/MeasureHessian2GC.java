...
/**
 * -Xmx2G -Xms2G -Xmn1G -javaagent:/your/path/to/aprof.jar
 */
public class MeasureHessian2GC {
    ...
    @Test
    public void testDeserializeOriginal() throws IOException {
        long result = 0L;
        for (int i = 0; i < TEST_TIMES; i++) {
            Hessian2Input mH2i = new Hessian2Input(new ByteArrayInputStream(bytes));
            mH2i.setSerializerFactory(Hessian2SerializerFactory.SERIALIZER_FACTORY);
            result += mH2i.readObject(SimpleBO.class).hashCode();
        }
        System.out.println("testDeserializeOriginal(): TEST_TIMES=" + TEST_TIMES + ", result=" + result);
    }

    @Test
    public void testDeserializeOptimize() throws IOException {
        long result = 0L;
        for (int i = 0; i < TEST_TIMES; i++) {
        ...
        System.out.println("testDeserializeOptimize(): TEST_TIMES=" + TEST_TIMES + ", result=" + result);
    }

    @Test
    public void testSerializeOriginal() throws IOException {
        long result = 0L;
        for (int i = 0; i < TEST_TIMES; i++) {
            ByteArrayOutputStream outputStream = new ByteArrayOutputStream(256);
            ...
            mH2o.writeObject(simpleBO);
            mH2o.flush();
            result += outputStream.toByteArray().length;
        }
        System.out.println("testSerializeOriginal(): TEST_TIMES=" + TEST_TIMES + ", result=" + result);
    }

    @Test
    public void testSerializeOptimize() throws IOException {
        long result = 0L;
        for (int i = 0; i < TEST_TIMES; i++) {
            ByteArrayOutputStream outputStream = new ByteArrayOutputStream(256);
            Hessian2Output mH2o = OUTPUT_TL.get();
        ...
        System.out.println("testSerializeOptimize(): TEST_TIMES=" + TEST_TIMES + ", result=" + result);
    }

    public static void main(String[] args) throws RunnerException {
        // 使用一个单独进程执行测试，执行5遍warmup，然后执行5遍测试
        Options opt = new OptionsBuilder()
                .include(MeasureHessian2GC.class.getSimpleName())
                .forks(1)
                .warmupIterations(3)
